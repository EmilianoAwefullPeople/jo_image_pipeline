...
`created` → `queued` → `inventorying` → `extracting` → `thumbnailing` → `grouping` → `evaluating` → `refining` → `reviewing` → `complete`, or `failed` with a `failure_detail`. The client polls `GET /api/runs/{id}` for the current stage and results.

The run payload carries both `groups` and `baseline_groups`. `baseline_groups` is what capture time, GPS and OpenCV alone produced; `groups` is that result after the visual model re-elected representatives on composition rather than sharpness, dropped anything it read as not worth keeping, and flagged screenshots without removing them, and after the moment review (`resources/explainers/moment_review.md`) read each outing's descriptions as text and joined or split the rule-based moments where the text argued for it. Each reviewed moment carries `evidence.review` with its title, reason and what changed, and `llm.review` in the payload carries the review summary and records.

Once a run is complete, the moments panel offers a **Group by** drop-down of grouping styles (`GET /api/styles`: Moments, Memories, Landmark tour, Foodie tour, Location highlights, Enjoyable moments) and a Regroup button. `POST /api/runs/{id}/style` with `{"style": id}` queues a text-only review of the same per-image descriptions under that style (409 before the run is complete or while a regrouping is in flight, 400 for an unknown style); the run shows `reviewing` again and `groups` becomes that style's groups, each with `evidence.review.title`, `about` and `why`. Topic styles leave photos out: `llm.review.unassigned` lists them and the page says how many are not in this view. The photos are never sent again, and every style is cached so switching back is free. Showing both is the point — it is the only way a viewer can see what the model contributed rather than taking it on trust. When no API key is configured the two are identical and the demo still works, returning metadata and grouping only.
